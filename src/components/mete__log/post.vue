<template>
    <div class="post__container__container__generale__router">
        <div class="post__principale">
            <h2  @click="$router.push({ path: '/profilo:' + this.arrayUtente.utente__id})">{{ this.arrayUtente.username }}</h2>
            <h1 class="h1__big">{{ this.arrayUtente.luogo }}</h1>
            <div  v-if="this.arrayImmagini.length != 0" class="container__image__post">
                <v-carousel :show-arrows="false">
                    <v-carousel-item v-for="arrayImmagini, index in this.arrayImmagini" :key="index"
                        :src="arrayImmagini" cover>
                    </v-carousel-item>
                </v-carousel>
            </div>
            <div>
            ...
            <div class="navbar__laterale__post__inside ods__mini__card">
                ...
                <div>
                    <div class="suggerimenti__account">
                        <h2>Commenti</h2>
                        <ul v-for="commento in arrayCommenti" :key="commento.id">
                    ...
                    <div>
                        ...
                        <button
                            @click="CreaCommento(this.$route.params.postId), TakeCommenti(this.$route.params.postId)">Posta</button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        checkIfLiked: async function () {
            const docRefUt = doc(DataService.dbEx(), 'likes', this.user + "_" + this.arrayUtente.id);
            const docSnap = await getDoc(docRefUt);
        ...
        unlikeImage: async function () {
            deleteDoc(doc(DataService.dbEx(), 'likes', this.user + "_" + this.arrayUtente.id)).then(() =>{
                this.checkIfLiked()
            });
            
        },
        likeImage: async function () {
            const docRef = await setDoc(doc(DataService.dbEx(), 'likes', this.user + "_" + this.arrayUtente.id), {
                userId: this.user,
                postId: this.arrayUtente.id
            })
            this.checkIfLiked()
        }
    },
